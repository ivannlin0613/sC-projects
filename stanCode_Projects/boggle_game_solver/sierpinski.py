...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	"""
	This function draws the sierpinski triangle by what order user demands.
	"""
	# Base case
	if order == 0:
		pass
		...
		r_line = GLine(upper_left_x+length, upper_left_y, upper_left_x+length*0.5, upper_left_y+length*0.866)
		l_line = GLine(upper_left_x+length*0.5, upper_left_y+length*0.866, upper_left_x, upper_left_y)
		window.add(up_line)
		window.add(r_line)
		window.add(l_line)
